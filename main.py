...
class HexBoard(tk.Canvas):
    ...
    def add_vector_as_image(self, row, col, vector):
        ...
        pixel_size = self.hex_size * 1.2 / size  # Scale to fit within hexagon

        for i in range(size):
            for j in range(size):
                color_intensity = int(vector[i * size + j])
                color = f'#{color_intensity:02x}{color_intensity:02x}{color_intensity:02x}'
                x0 = x_center + (j - size / 2) * pixel_size
                y0 = y_center + (i - size / 2) * pixel_size
                x1 = x0 + pixel_size
                y1 = y0 + pixel_size
                self.create_rectangle(x0, y0, x1, y1, outline=color, fill=color)
